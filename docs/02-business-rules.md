...
- `pay_later`: khách bắt buộc chọn 2 hoặc 4 tháng và xác nhận nghĩa vụ trước khi đặt đơn. Đây là trả toàn bộ vào ngày đến hạn, không phải trả góp. Khoản trả sau có trạng thái riêng; hệ thống chỉ nhắc hạn và không tự động trừ PayPal, Clare Pay, PayOS hay tài khoản ngân hàng.
- Khi đến hạn/quá hạn, khách chủ động chọn PayPal mô phỏng, Clare Pay hoặc PayOS. PayPal mô phỏng chỉ hoàn tất sau nút xác nhận; Clare Pay khóa ví và kiểm tra đủ số dư trong transaction; PayOS chỉ hoàn tất qua webhook/API đã xác minh.
- Nạp Clare Pay qua PayOS dùng loại giao dịch riêng với thanh toán đơn và thanh toán khoản trả sau. Webhook lặp phải idempotent và không được cộng số dư lần hai.
