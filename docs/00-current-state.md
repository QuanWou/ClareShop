...
- Checkout API có endpoint báo giá và tạo đơn; hỗ trợ COD hoặc chuyển khoản VietQR với mã QR động theo tổng tiền và mã đơn. Phí ship đang là ước tính động cho đến khi kết nối GHN/GHTK thật.
- Checkout web bắt buộc khách đăng nhập bằng tài khoản đang hoạt động; form lấy email snapshot từ tài khoản ở server, gắn mọi đơn mới với `user_id` và chỉ chủ đơn được xem trang xác nhận qua signed URL. Form có địa chỉ, ước tính phí ship và lựa chọn COD/VietQR; QR VietQR chỉ hiển thị hướng dẫn thanh toán, không tự đánh dấu đơn đã thanh toán.
